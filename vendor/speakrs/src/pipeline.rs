...
pub use config::{
    CUDA_SEGMENTATION_STEP_SECONDS, FAST_SEGMENTATION_STEP_SECONDS, FRAME_DURATION_SECONDS,
...
impl<'a> PipelineRunner<'a> {
    ...
    fn inference_path(&self) -> InferencePath {
        if self.seg_model.mode().is_cuda() {
    ...
    fn run_batch(
        ...
        // per-file sequential with post-inference overlap
        files
            .iter()
