...
use ttfx_rs::engine::animation::{SyncMetric, VisualParams};
use ttfx_rs::engine::character::CharId;
use ttfx_rs::engine::ctx::{Clock, EngineCtx, NoopHooks};
use ttfx_rs::engine::events::{CallerKey, Event, EventAction};
use ttfx_rs::engine::terminal::{CharacterFilter, CharacterSort, TerminalConfig};
use ttfx_rs::utils::easing::Easing;
use ttfx_rs::utils::geometry::Coord;
use ttfx_rs::utils::graphics::{Color, ColorPair, Gradient};
...
fn chars(ctx: &mut EngineCtx, n: usize) -> Vec<CharId> {
    ...
    ctx.terminal
        .get_characters(&mut rng, filter, CharacterSort::TopToBottomLeftToRight)[..n]
        .to_vec()
}
...
fn snapshot(ctx: &mut EngineCtx, log: &mut Vec<String>, tick: i64, ids: &[CharId]) {
...
fn run_ticks(ctx: &mut EngineCtx, log: &mut Vec<String>, ids: &[CharId], n: i64, start: i64) {
...
fn scenario_scenes(log: &mut Vec<String>) {
    ...
    ctx.activate_scene(&mut NoopHooks, b, "looping");
    {
        ...
        let uses = ch.uses_input_preexisting_colors;
        ch.animation
            .new_scene(false, None, Some(Easing::InOutCubic), "eased", uses);
        ...
        scene
            .apply_gradient_to_symbols(&["*".into(), "+".into(), "o".into()], 2, Some(&grad), None)
            .unwrap();
    }
...
fn scenario_synced_scene(log: &mut Vec<String>) {
    ...
    let mut ctx = make_ctx();
    let ids = chars(&mut ctx, 2);
    for (idx, (sync, pid)) in [(SyncMetric::Step, "sp"), (SyncMetric::Distance, "dp")]
        .iter()
        .enumerate()
    {
        let id = ids[idx];
