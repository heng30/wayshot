...
impl EngineCtx {
    ...
    fn path_step(&mut self, hooks: &mut dyn EffectHooks, id: CharId, path_id: &str) -> Coord {
        ...
        match &seg.end.bezier_control {
            Some(control) => {
                geometry::find_coord_on_bezier_curve(seg.start.coord, control, seg.end.coord, t)
            }
    ...
    /// Advance the animation clock by one frame and return the formatted
    /// output string (the effects' next_frame contract). Image rendering
    /// ignores the string and reads [`crate::engine::terminal::Terminal::frame_cells`]
    /// instead.
