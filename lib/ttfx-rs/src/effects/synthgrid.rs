    ...
    fn default() -> Self {
        Self {
            grid_gradient_stops: vec![
                parse_color("CC00CC").unwrap(),
                parse_color("ffffff").unwrap(),
            ],
            grid_gradient_steps: vec![parse_gradient_steps("12").unwrap()],
            grid_gradient_direction: parse_gradient_direction("diagonal").unwrap(),
            text_gradient_stops: vec![
                parse_color("8A008A").unwrap(),
                parse_color("00D1FF").unwrap(),
                parse_color("FFFFFF").unwrap(),
            ],
            text_gradient_steps: vec![parse_gradient_steps("12").unwrap()],
            text_gradient_direction: parse_gradient_direction("vertical").unwrap(),
            grid_row_symbol: parse_symbol("─").unwrap(),
            grid_column_symbol: parse_symbol("│").unwrap(),
        ...
        for &row_index_value in &row_indexes {
            ...
            for &column_index in &column_indexes {
                ...
                if !characters_in_block.is_empty() {
                    self.pending_groups
                        .push((self.pending_groups.len() as i64, characters_in_block));
                }
