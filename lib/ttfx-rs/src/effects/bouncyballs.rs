...
use crate::effects::common::{
    parse_easing, parse_gradient_direction, parse_gradient_steps, parse_symbol,
};
...
impl Default for BouncyBallsConfig {
    fn default() -> Self {
        Self {
            ball_colors: vec![
                parse_color("d1f4a5").unwrap(),
                parse_color("96e2a4").unwrap(),
                parse_color("5acda9").unwrap(),
            ],
            ball_symbols: vec![
                parse_symbol("*").unwrap(),
                parse_symbol("o").unwrap(),
                parse_symbol("O").unwrap(),
                parse_symbol("0").unwrap(),
                parse_symbol(".").unwrap(),
            ],
            ball_delay: 4,
            movement_speed: 0.45,
            movement_easing: parse_easing("out_bounce").unwrap(),
            final_gradient_stops: vec![
                parse_color("f8ffae").unwrap(),
                parse_color("43c6ac").unwrap(),
            ],
        ...
        for id in characters {
            ...
            if dynamic {
                let fg_gradient = match &input_fg {
                    Some(fg) => Some(
                        Gradient::with_steps(&[color.clone(), fg.clone()], 10, false)
                            .map_err(EngineError::Other)?,
                    ),
                    None => None,
                };
                let bg_gradient = match &input_bg {
                    Some(bg) => Some(
                        Gradient::with_steps(&[color.clone(), bg.clone()], 10, false)
                            .map_err(EngineError::Other)?,
                    ),
                    None => None,
                };
                ...
                let char_final_gradient =
                    Gradient::with_steps(&[color.clone(), final_color], 10, false)
                        .map_err(EngineError::Other)?;
                let ch = &mut ctx.terminal.arena[id.0 as usize];
                ch.animation
                    .scenes
            ...
            if !self.pending_chars.is_empty() {
                if self.ball_delay == 0 {
                    for _ in 0..ctx.rng.randint(2, 6) {
                        ...
                        let index =
                            ctx.rng.randint(0, self.pending_chars.len() as i64 - 1) as usize;
