...
impl Default for PourConfig {
    fn default() -> Self {
        Self {
            pour_direction: parse_pour_direction("down").unwrap(),
            pour_speed: 2,
            movement_speed_range: parse_positive_float_range("0.4-0.6").unwrap(),
            gap: 1,
            starting_color: parse_color("ffffff").unwrap(),
            ...
            movement_easing: parse_easing("in_quad").unwrap(),
        }
    }
}
                ...
                let final_colors = self.character_final_color_map.get(&id).unwrap().clone();
                {
                    ...
                    if dynamic {
                        ...
                        let fg_gradient = match &final_fg_color {
                            Some(c) => Some(
                                Gradient::with_steps(
                                    &[self.config.starting_color.clone(), c.clone()],
                                    10,
                                    false,
                                )
                                .map_err(EngineError::Other)?,
