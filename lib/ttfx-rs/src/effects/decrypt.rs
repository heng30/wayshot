    ...
    fn default() -> Self {
        Self {
            ...
            ciphertext_colors: vec![
                parse_color("008000").unwrap(),
                parse_color("00cb00").unwrap(),
                parse_color("00ff00").unwrap(),
            ],
            final_gradient_stops: vec![parse_color("eda000").unwrap()],
    ...
    fn make_decrypting_animation_scenes(
        ...
        if dynamic {
            let fg_gradient = match &input_fg {
                Some(fg) => Some(
                    Gradient::with_steps(&[white.clone(), fg.clone()], 10, false)
                        .map_err(EngineError::Other)?,
                ),
                None => None,
            };
            let bg_gradient = match &input_bg {
                Some(bg) => Some(
                    Gradient::with_steps(&[white.clone(), bg.clone()], 10, false)
                        .map_err(EngineError::Other)?,
                ),
                None => None,
            };
