...
impl Default for OrbittingVolleyConfig {
    fn default() -> Self {
        Self {
            top_launcher_symbol: parse_symbol("█").unwrap(),
            right_launcher_symbol: parse_symbol("█").unwrap(),
            bottom_launcher_symbol: parse_symbol("█").unwrap(),
            left_launcher_symbol: parse_symbol("█").unwrap(),
            launcher_movement_speed: 0.8,
            character_movement_speed: 1.5,
            volley_size: 0.03,
            launch_delay: 30,
            character_easing: parse_easing("out_sine").unwrap(),
...
impl Effect for OrbittingVolley {
    ...
    fn next_frame(&mut self, ctx: &mut EngineCtx) -> Option<String> {
        if self.launchers.iter().any(|l| !l.magazine.is_empty()) || ctx.active_characters.len() > 1
        {
            ...
            {
                let current_coord = ctx.terminal.arena[main_character.0 as usize]
                    .motion
                    .current_coord;
                let color = self
                    .launcher_gradient_coordinate_map
            ...
            if self.delay == 0 {
                for launcher_index in 0..self.launchers.len() {
                    ...
                    let characters_to_launch = std::cmp::max(
                        ((self.config.volley_size * ctx.terminal.input_characters.len() as f64)
                            / 4.0) as i64,
                        1,
                    );
