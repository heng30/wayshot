...
impl Default for BlackholeConfig {
    fn default() -> Self {
        Self {
            blackhole_color: parse_color("ffffff").unwrap(),
            star_colors: vec![
                parse_color("ffcc0d").unwrap(),
                parse_color("ff7326").unwrap(),
                parse_color("ff194d").unwrap(),
                parse_color("bf2669").unwrap(),
                parse_color("702a8c").unwrap(),
                parse_color("049dbf").unwrap(),
            ],
            ...
            final_gradient_steps: vec![parse_gradient_steps("9").unwrap()],
...
impl Blackhole {
    ...
    fn prepare_blackhole(&mut self, ctx: &mut EngineCtx) -> Result<(), EngineError> {
        ...
        for color in &starfield_colors {
            gradient_map.push(
                Gradient::with_steps(
                    &[color.clone(), Color::from_hex("#000000").unwrap()],
                    10,
                    false,
                )
                .map_err(EngineError::Other)?,
        ...
        for (position_index, &id) in self.blackhole_chars.iter().enumerate() {
            ...
            };
            {
                ...
                let blackhole_scn =
                    ch.animation
                        .new_scene(false, None, None, "blackhole", uses_pre);
                ch.animation
                    .scenes
                    ...
                    .add_frame(
                        ...
                        VisualParams {
                            colors: Some(ColorPair::new(
                                Some(self.config.blackhole_color.clone()),
                                None,
                            )),
                            ..Default::default()
                        },
            ...
            if dynamic && ctx.preexisting_colors_present {
                if input_fg.is_none() && input_bg.is_none() {
                    ...
                    let cooling_gradient_fg = match &input_fg {
                        Some(fg) => Some(
                            Gradient::with_steps(
                                &[explode_star_color.clone(), fg.clone()],
                                10,
                                false,
                            )
                            .map_err(EngineError::Other)?,
                    ...
                    let cooling_gradient_bg = match &input_bg {
                        Some(bg) => Some(
                            Gradient::with_steps(
                                &[explode_star_color.clone(), bg.clone()],
                                10,
                                false,
                            )
                            .map_err(EngineError::Other)?,
            ...
            match self.phase {
                ...
                Phase::Consuming => {
                    if !self.awaiting_consumption_chars.is_empty() {
                        ...
                        let blackhole_set: HashSet<CharId> =
