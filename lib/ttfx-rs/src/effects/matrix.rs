...
use crate::effects::common::{
    parse_gradient_direction, parse_gradient_steps, parse_positive_int_range, parse_symbol,
};
...
impl Default for MatrixConfig {
    fn default() -> Self {
        Self {
            highlight_color: parse_color("dbffdb").unwrap(),
            rain_color_gradient: vec![
                parse_color("92be92").unwrap(),
                parse_color("185318").unwrap(),
            ],
            rain_symbols: vec![
                parse_symbol("2").unwrap(),
                parse_symbol("5").unwrap(),
                parse_symbol("9").unwrap(),
                parse_symbol("8").unwrap(),
                parse_symbol("Z").unwrap(),
                parse_symbol("*").unwrap(),
                parse_symbol(")").unwrap(),
                parse_symbol(":").unwrap(),
                parse_symbol(".").unwrap(),
                ...
                parse_symbol("¦").unwrap(),
                parse_symbol("|").unwrap(),
                parse_symbol("_").unwrap(),
                parse_symbol("ｦ").unwrap(),
                parse_symbol("ｱ").unwrap(),
                parse_symbol("ｳ").unwrap(),
                parse_symbol("ｴ").unwrap(),
                parse_symbol("ｵ").unwrap(),
                parse_symbol("ｶ").unwrap(),
                parse_symbol("ｷ").unwrap(),
                parse_symbol("ｹ").unwrap(),
                parse_symbol("ｺ").unwrap(),
                parse_symbol("ｻ").unwrap(),
                parse_symbol("ｼ").unwrap(),
                parse_symbol("ｽ").unwrap(),
                parse_symbol("ｾ").unwrap(),
                parse_symbol("ｿ").unwrap(),
                parse_symbol("ﾀ").unwrap(),
                parse_symbol("ﾂ").unwrap(),
                parse_symbol("ﾃ").unwrap(),
                parse_symbol("ﾅ").unwrap(),
                parse_symbol("ﾆ").unwrap(),
                parse_symbol("ﾇ").unwrap(),
                parse_symbol("ﾈ").unwrap(),
                parse_symbol("ﾊ").unwrap(),
                parse_symbol("ﾋ").unwrap(),
                parse_symbol("ﾎ").unwrap(),
                parse_symbol("ﾏ").unwrap(),
                parse_symbol("ﾐ").unwrap(),
                parse_symbol("ﾑ").unwrap(),
                parse_symbol("ﾒ").unwrap(),
                parse_symbol("ﾓ").unwrap(),
                parse_symbol("ﾔ").unwrap(),
                parse_symbol("ﾕ").unwrap(),
                parse_symbol("ﾗ").unwrap(),
                parse_symbol("ﾘ").unwrap(),
                parse_symbol("ﾜ").unwrap(),
            ],
            rain_fall_delay_range: parse_positive_int_range("2-15").unwrap(),
            rain_column_delay_range: parse_positive_int_range("3-9").unwrap(),
            rain_time: 15,
            ...
            final_gradient_stops: vec![
                parse_color("92be92").unwrap(),
                parse_color("336b33").unwrap(),
            ],
            final_gradient_steps: vec![parse_gradient_steps("12").unwrap()],
            final_gradient_frames: 3,
            final_gradient_direction: parse_gradient_direction("radial").unwrap(),
        }
    }
}
...
fn set_appearance(ctx: &mut EngineCtx, id: CharId, symbol: &str, colors: ColorPair) {
    ...
    ch.animation
        .set_appearance(&input_symbol, uses_pre, Some(symbol), Some(colors));
}
            ...
            if !self.pending_characters.is_empty() {
                ...
                let last_is_highlight = {
                    ...
                    visual.colors.as_ref().is_some_and(|colors| {
                        colors.fg_color.as_ref() == Some(&config.highlight_color)
                    })
                };
            ...
            match (next_symbol, next_color) {
                (Some(symbol), Some(color)) => {
                    set_appearance(
                        ...
                        character,
                        symbol,
                        ColorPair::new(Some(color.clone()), None),
                    );
                }
                (Some(symbol), None) => {
...
impl Effect for Matrix {
    fn build(&mut self, ctx: &mut EngineCtx) -> Result<(), EngineError> {
        ...
        self.rain_colors = Gradient::with_steps(&self.config.rain_color_gradient, 6, false)
        ...
        for character in characters {
            ...
            self.character_final_color_map
                .insert(character, final_colors.clone());
            ...
            if dynamic {
                let fg_gradient = match &final_fg_color {
                    Some(fg) => Some(
                        Gradient::with_steps(
                            &[self.config.highlight_color.clone(), fg.clone()],
                            8,
                            false,
                        )
                        .map_err(EngineError::Other)?,
                ...
                let bg_gradient = match &final_bg_color {
                    Some(bg) => Some(
                        Gradient::with_steps(
                            &[self.config.highlight_color.clone(), bg.clone()],
                            8,
                            false,
                        )
                        .map_err(EngineError::Other)?,
                ...
                let resolve_gradient = Gradient::with_steps(
                    &[self.config.highlight_color.clone(), final_fg_color],
                    8,
                    false,
                )
                .map_err(EngineError::Other)?;
        ...
        for mut column_chars in ctx
            .terminal
            .get_characters_grouped(all_chars_filter, CharacterGroup::ColumnLeftToRight)
        {
                    ...
                    if self.resolve_delay == 0 {
                        for _ in 0..ctx.rng.randint(1, 4) {
                            if !self.columns[column_index].visible_characters.is_empty() {
                                ...
                                let input_symbol = ctx.terminal.arena[next_char.0 as usize]
                                    .input_symbol
                                    .clone();
