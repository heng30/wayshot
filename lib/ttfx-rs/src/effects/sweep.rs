...
impl Default for SweepConfig {
    fn default() -> Self {
        Self {
            ...
            first_sweep_direction: parse_character_group("column_right_to_left").unwrap(),
            second_sweep_direction: parse_character_group("column_left_to_right").unwrap(),
            final_gradient_stops: vec![
                parse_color("8A008A").unwrap(),
                parse_color("00D1FF").unwrap(),
                parse_color("ffffff").unwrap(),
            ],
            final_gradient_steps: vec![parse_gradient_steps("8").unwrap()],
    ...
    fn build(&mut self, ctx: &mut EngineCtx) -> Result<(), EngineError> {
        ...
        for id in characters {
            ...
            }
            {
                ...
                ch.animation
                    .new_scene(false, None, None, "initial_sweep", uses_pre);
            }
            ...
            }
            {
                ...
                ch.animation
                    ...
                    .map_err(EngineError::Other)?;
                ch.animation
                    .new_scene(false, None, None, "second_sweep", uses_pre);
            }
            for symbol in &self.config.sweep_symbols {
        ...
        let groups_first_sweep = ctx
            .terminal
            .get_characters_grouped(fills_filter, self.config.first_sweep_direction);
        self.easer = Some(SequenceEaser::new(
            groups_first_sweep,
            Easing::InOutCirc,
            100,
        ));
        self.groups_second_sweep = ctx
            .terminal
            .get_characters_grouped(fills_filter, self.config.second_sweep_direction);
        Ok(())
    }
