...
impl Default for SprayConfig {
    fn default() -> Self {
        Self {
            spray_position: parse_spray_position("e").unwrap(),
            spray_volume: 0.005,
            movement_speed_range: parse_positive_float_range("0.6-1.4").unwrap(),
            movement_easing: parse_easing("out_expo").unwrap(),
    ...
    fn build(&mut self, ctx: &mut EngineCtx) -> Result<(), EngineError> {
        ...
        for id in characters {
            ...
            if dynamic {
                ...
                let spray_gradient = Gradient::with_steps(&[start_color, final_fg], 7, false)
                    .map_err(EngineError::Other)?;
                let ch = &mut ctx.terminal.arena[id.0 as usize];
                ch.animation
                    .scenes
