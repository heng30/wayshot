...
impl Default for BurnConfig {
    fn default() -> Self {
        Self {
            starting_color: parse_color("837373").unwrap(),
            burn_colors: vec![
                parse_color("ffffff").unwrap(),
                parse_color("fff75d").unwrap(),
                parse_color("fe650d").unwrap(),
                parse_color("8A003C").unwrap(),
                parse_color("510100").unwrap(),
            ],
            smoke_chance: 0.5,
            final_gradient_stops: vec![
                parse_color("00c3ff").unwrap(),
                parse_color("ffff1c").unwrap(),
            ],
...
impl Effect for Burn {
    fn build(&mut self, ctx: &mut EngineCtx) -> Result<(), EngineError> {
        ...
        let fire_gradient = Gradient::with_steps(&self.config.burn_colors, 10, false)
            .map_err(EngineError::Other)?;
        ...
        for &id in &characters {
            ...
            };
            {
                ...
                ch.animation.set_appearance(
                    ...
                    Some(&input_symbol),
                    Some(ColorPair::new(
                        Some(self.config.starting_color.clone()),
                        None,
                    )),
                );
            }
            ...
            if dynamic {
                ...
                let char_gradient = Gradient::with_steps(&[fire_last, final_color], 8, false)
                    .map_err(EngineError::Other)?;
                let ch = &mut ctx.terminal.arena[id.0 as usize];
                let scene = ch.animation.scenes.get_mut(&final_color_scn).unwrap();
