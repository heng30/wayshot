    ...
    fn default() -> Self {
        Self {
            ...
            wave_gradient_stops: vec![
                ...
                parse_color("ffb102").unwrap(),
                parse_color("31a0d4").unwrap(),
                parse_color("ffb102").unwrap(),
                parse_color("f0ff65").unwrap(),
            ],
            wave_gradient_steps: vec![parse_gradient_steps("6").unwrap()],
            wave_count: 7,
            wave_length: 2,
            wave_direction: parse_wave_direction("column_left_to_right").unwrap(),
            wave_easing: parse_easing("in_out_sine").unwrap(),
...
impl Effect for Waves {
    fn build(&mut self, ctx: &mut EngineCtx) -> Result<(), EngineError> {
        ...
        for id in characters {
            ...
            if dynamic {
                ...
                let final_scene_gradient = Gradient::new(
                    &[
                        wave_gradient.spectrum.last().unwrap().clone(),
                        final_fg_color,
                    ],
                    &self.config.final_gradient_steps,
                    false,
        ...
        for column in ctx
            .terminal
            .get_characters_grouped(CharacterFilter::default(), self.config.wave_direction)
        {
