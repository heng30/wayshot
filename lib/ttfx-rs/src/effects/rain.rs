...
use crate::effects::common::{
    parse_easing, parse_gradient_direction, parse_gradient_steps, parse_positive_float_range,
    parse_symbol,
};
...
impl Default for RainConfig {
    fn default() -> Self {
        Self {
            rain_colors: vec![
                parse_color("00315C").unwrap(),
                parse_color("004C8F").unwrap(),
                parse_color("0075DB").unwrap(),
                parse_color("3F91D9").unwrap(),
                parse_color("78B9F2").unwrap(),
                parse_color("9AC8F5").unwrap(),
                parse_color("B8D8F8").unwrap(),
                parse_color("E3EFFC").unwrap(),
            ],
            movement_speed: parse_positive_float_range("0.33-0.57").unwrap(),
            rain_symbols: vec![
                parse_symbol("o").unwrap(),
                parse_symbol(".").unwrap(),
                parse_symbol(",").unwrap(),
                parse_symbol("*").unwrap(),
                parse_symbol("|").unwrap(),
            ...
            final_gradient_stops: vec![
                parse_color("488bff").unwrap(),
                parse_color("b2e7de").unwrap(),
                parse_color("57eaf7").unwrap(),
            ],
            final_gradient_steps: vec![parse_gradient_steps("12").unwrap()],
            final_gradient_direction: parse_gradient_direction("diagonal").unwrap(),
            movement_easing: parse_easing("in_quart").unwrap(),
        }
    }
}
        ...
        for id in characters {
            ...
            if dynamic {
                ...
                let raindrop_gradient =
                    Gradient::with_steps(&[raindrop_color.clone(), final_fg], 7, false)
                        .map_err(EngineError::Other)?;
                let ch = &mut ctx.terminal.arena[id.0 as usize];
                ch.animation
                    .scenes
            ...
            let speed = ctx
                .rng
                .uniform(self.config.movement_speed.0, self.config.movement_speed.1);
            let input_path = {
                let ch = &mut ctx.terminal.arena[id.0 as usize];
                ch.motion
