...
impl Default for LaserEtchConfig {
    fn default() -> Self {
        Self {
            etch_pattern: parse_etch_pattern("algorithm").unwrap(),
            etch_speed: 1,
            etch_delay: 1,
            cool_gradient_stops: vec![
                parse_color("ffe680").unwrap(),
                parse_color("ff7b00").unwrap(),
            ],
            laser_gradient_stops: vec![
                parse_color("ffffff").unwrap(),
                parse_color("376cff").unwrap(),
            ],
            spark_gradient_stops: vec![
                parse_color("ffffff").unwrap(),
                parse_color("ffe680").unwrap(),
                parse_color("ff7b00").unwrap(),
                parse_color("1a0900").unwrap(),
            ],
    ...
    fn make_laser(&mut self, ctx: &mut EngineCtx) -> Result<Laser, EngineError> {
        let mut laser_gradient: VecDeque<Color> =
            Gradient::new(&self.config.laser_gradient_stops, &[6], true, true)
                ...
                .into();
        let spark_gradient =
            Gradient::new(&self.config.spark_gradient_stops, &[3, 8], false, false)
                .map_err(EngineError::Other)?;
    ...
    fn build(&mut self, ctx: &mut EngineCtx) -> Result<(), EngineError> {
        ...
        for id in characters {
            ...
            if dynamic {
                if final_fg_color.is_some() || final_bg_color.is_some() {
                    ...
                    white_cooldown = Some(
                        Gradient::with_steps(
                            &[cool_last.clone(), Color::from_hex("ffffff").unwrap()],
                            8,
                            false,
                        )
                        .map_err(EngineError::Other)?,
        ...
        match self.config.etch_pattern {
            ...
            EtchPattern::Algorithm => {
                let mut algo =
                    RecursiveBacktracker::new(ctx, None, true).map_err(EngineError::Other)?;
                while !algo.complete {
                    algo.step(ctx);
                }
