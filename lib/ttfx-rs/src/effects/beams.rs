...
impl Default for BeamsConfig {
    fn default() -> Self {
        Self {
            ...
            beam_delay: 6,
            beam_row_speed_range: parse_positive_int_range("15-60").unwrap(),
            beam_column_speed_range: parse_positive_int_range("9-15").unwrap(),
            beam_gradient_stops: vec![
                parse_color("ffffff").unwrap(),
                parse_color("00D1FF").unwrap(),
                parse_color("8A008A").unwrap(),
            ],
            beam_gradient_steps: vec![
                parse_gradient_steps("2").unwrap(),
                parse_gradient_steps("6").unwrap(),
            ],
...
impl Effect for Beams {
    fn build(&mut self, ctx: &mut EngineCtx) -> Result<(), EngineError> {
        ...
        for row in ctx
            .terminal
            .get_characters_grouped(all_chars_filter, CharacterGroup::RowTopToBottom)
        {
            ...
            .filter(|group| group.direction == Direction::Row)
        {
            for &id in &group.characters {
                ...
                };
                {
                    ...
                    ch.animation
                        .new_scene(false, None, None, "beam_column", uses_pre);
                    ch.animation
