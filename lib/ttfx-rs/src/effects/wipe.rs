...
use crate::effects::common::{
    parse_character_group, parse_easing, parse_gradient_direction, parse_gradient_steps,
};
...
impl Default for WipeConfig {
    fn default() -> Self {
        Self {
            wipe_direction: parse_character_group("diagonal_top_left_to_bottom_right").unwrap(),
            wipe_delay: 0,
            wipe_ease: parse_easing("in_out_circ").unwrap(),
            final_gradient_stops: vec![
                parse_color("833ab4").unwrap(),
                parse_color("fd1d1d").unwrap(),
                parse_color("fcb045").unwrap(),
            ],
...
impl Effect for Wipe {
    fn build(&mut self, ctx: &mut EngineCtx) -> Result<(), EngineError> {
        let groups = ctx
            .terminal
            .get_characters_grouped(CharacterFilter::default(), self.config.wipe_direction);
