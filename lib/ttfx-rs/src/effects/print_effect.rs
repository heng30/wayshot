...
impl Default for PrintConfig {
    fn default() -> Self {
        Self {
            ...
            print_speed: 2,
            print_head_easing: parse_easing("in_out_quad").unwrap(),
            final_gradient_stops: vec![
                parse_color("02b8bd").unwrap(),
                parse_color("c1f0e3").unwrap(),
                parse_color("00ffa0").unwrap(),
            ],
            ...
            if dynamic {
                ...
                if fg_gradient.is_some() || bg_gradient.is_some() {
                    ...
                    let head_gradient = Gradient::with_steps(
                        &[typing_head_color.clone(), typing_head_color.clone()],
                        4,
                        false,
                    )
                    .map_err(EngineError::Other)?;
                ...
                let color_gradient =
                    Gradient::with_steps(&[typing_head_color.clone(), final_fg], 5, false)
                        .map_err(EngineError::Other)?;
                let ch = &mut ctx.terminal.arena[id.0 as usize];
                ch.animation
                    .scenes
...
impl Effect for Print {
    fn build(&mut self, ctx: &mut EngineCtx) -> Result<(), EngineError> {
        ...
        for input_row in input_rows {
            // The canvas is padded with fill characters across the whole
            // drawing area (upstream only fills inside the input bounding
            // box), so skip rows that contain nothing but fill spaces.
            let all_spaces = input_row
                .iter()
                .all(|&id| ctx.terminal.arena[id.0 as usize].input_symbol == " ");
            if all_spaces {
                continue;
                ...
                .is_some()
            {
                ...
                for _ in 0..count {
                    if let Some(next_char) = self.current_row.type_char() {
                        ...
                        self.last_column =
                            ctx.terminal.arena[next_char.0 as usize].input_coord.column;
                    }
                }
            } else {
