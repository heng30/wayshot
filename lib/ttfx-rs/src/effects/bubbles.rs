...
impl Default for BubblesConfig {
    fn default() -> Self {
        Self {
            rainbow: false,
            bubble_colors: vec![
                parse_color("d33aff").unwrap(),
                parse_color("7395c4").unwrap(),
                parse_color("43c2a7").unwrap(),
                parse_color("02ff7f").unwrap(),
            ],
            pop_color: parse_color("ffffff").unwrap(),
            bubble_speed: 0.5,
            bubble_delay: 20,
            pop_condition: parse_pop_condition("row").unwrap(),
            movement_easing: parse_easing("in_out_sine").unwrap(),
...
impl Bubbles {
    pub fn new(config: BubblesConfig) -> Self {
        ...
        let rainbow_gradient =
            Gradient::with_steps(&rainbow_stops, 5, false).expect("rainbow gradient");
        Bubbles {
            config,
            bubbles: Vec::new(),
    ...
    fn build(&mut self, ctx: &mut EngineCtx) -> Result<(), EngineError> {
        ...
        for id in characters {
            ...
            if dynamic {
                let fg_gradient = match &input_fg {
                    Some(fg) => Some(
                        Gradient::with_steps(
                            &[self.config.pop_color.clone(), fg.clone()],
                            8,
                            false,
                        )
                        .map_err(EngineError::Other)?,
                ...
                let bg_gradient = match &input_bg {
                    Some(bg) => Some(
                        Gradient::with_steps(
                            &[self.config.pop_color.clone(), bg.clone()],
                            8,
                            false,
                        )
                        .map_err(EngineError::Other)?,
        ...
        while !unbubbled_chars.is_empty() {
            ...
            if unbubbled_chars.len() < 5 {
                ...
                let count = ctx
                    .rng
                    .randint(5, std::cmp::min(unbubbled_chars.len() as i64, 20));
                for _ in 0..count {
