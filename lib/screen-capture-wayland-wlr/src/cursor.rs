...
struct CursorTrackerState {
    ...
    hole_radius: i32, // Radius of the transparent hole around cursor (30px)
}
...
impl CursorTracker {
    /// Create a rectangular approximation of a circle hole in the input region
    /// Returns a list of rectangles that form the donut shape (full area minus circle)
    fn create_donut_rectangles(
        ...
        // Add rectangles for each row
        for y in 0..surface_height {
            let dy = y - center_y;
            if dy.abs() > radius {
                // Outside the circle vertically - full width
                rectangles.push((0, y, surface_width, 1));
            } else {
                // Within the circle vertically - calculate horizontal bounds
                let dx_max = (radius_sq - dy * dy).max(0) as f64;
                let dx_max = dx_max.sqrt() as i32;
                
                let left_bound = (center_x - dx_max).max(0);
                let right_bound = (center_x + dx_max).min(surface_width);
                
                // Add left rectangle if there's space
                if left_bound > 0 {
                    rectangles.push((0, y, left_bound, 1));
                }
                
                // Add right rectangle if there's space
                if right_bound < surface_width {
                    rectangles.push((right_bound, y, surface_width - right_bound, 1));
                }
            }
        }
    ...
    /// Update input region for all surfaces to create a hole around cursor position
    fn update_input_regions(&mut self, cursor_x: i32, cursor_y: i32) -> Result<(), CursorError> {
        log::debug!("Updating input regions for global position ({}, {})", cursor_x, cursor_y);
        
        for (surface_idx, (_output, output_info)) in self.state.outputs.iter().enumerate() {
            // Calculate cursor position relative to this surface
            let surface_x = cursor_x - output_info.x;
            let surface_y = cursor_y - output_info.y;
            
            log::debug!("Surface {}: surface coords ({}, {}), bounds: {}x{} at ({}, {})", 
                       surface_idx, surface_x, surface_y, 
                       output_info.width, output_info.height, output_info.x, output_info.y);
            
            // Skip if cursor is not on this output
            if surface_x < 0 || surface_y < 0 || 
               surface_x >= output_info.width || surface_y >= output_info.height {
                log::debug!("Skipping surface {} - cursor not on this output", surface_idx);
                continue;
            }
            
            // Always update for now - we'll optimize this later if needed
            // The threshold logic was causing issues with responsiveness
            log::debug!("Updating input region at surface coords ({}, {})", surface_x, surface_y);
            ...
            // Create new input region with donut shape
            ...
            if !rectangles.is_empty() {
                let region = self.state.compositor.as_mut()
                ...
                log::debug!("Updated input region for surface {} at ({}, {})", surface_idx, surface_x, surface_y);
            }
        }
    ...
    pub fn new() -> Result<Self, CursorError> {
        ...
        // Set hole radius while preserving other state
    ...
    pub fn start_tracking(&mut self) -> Result<(), CursorError> {
        ...
        log::info!("Found {} outputs during start_tracking", self.state.outputs.len());
        for (i, (output, info)) in self.state.outputs.iter().enumerate() {
            log::info!("Output {}: {}x{} at ({}, {})", i, info.width, info.height, info.x, info.y);
        }

        // Wait for output information to be populated
        let mut attempts = 0;
        while self.state.outputs.iter().any(|(_, info)| info.width == 0 || info.height == 0) && attempts < 50 {
            self.queue.blocking_dispatch(&mut self.state)?;
            attempts += 1;
            log::debug!("Waiting for output info, attempt {}", attempts);
        }

        log::info!("After waiting, outputs count: {}", self.state.outputs.len());
        for (i, (output, info)) in self.state.outputs.iter().enumerate() {
            log::info!("Final Output {}: {}x{} at ({}, {})", i, info.width, info.height, info.x, info.y);
        }
        ...
        for (output, output_info) in self.state.outputs.iter() {
            let surface = self
                .state
                .compositor
                .as_mut()
            ...
            layer_surface.set_size(
                output_info.width.try_into().unwrap(),
                output_info.height.try_into().unwrap(),
            );
            layer_surface.set_margin(0, 0, 0, 0); // no margin
            layer_surface.set_exclusive_zone(0); // No exclusive zone
            layer_surface
            ...
            // Create empty input region to prevent receiving pointer events
            ...
            log::info!(
                "Configured layer surface: {}x{} at ({}, {}), overlay layer with empty input region",
                output_info.width,
                output_info.height,
                output_info.x,
                output_info.y
            ...
            let full_region = self.state.compositor.as_mut().unwrap().create_region(&self.queue.handle(), ());
            full_region.add(0, 0, output_info.width, output_info.height);
            surface.set_input_region(Some(&full_region));
            full_region.destroy();

            // Create buffer for this surface
            let buffer = if let Some(shm) = &self.state.shm {
                let width = output_info.width as u32;
                let height = output_info.height as u32;
                let stride = width * 4;
                let size = (stride * height) as i32;

                let temp_file = tempfile::tempfile().map_err(|e| {
                    CursorError::PointerFailed(format!("Failed to create temp file: {}", e))
                })?;

                temp_file.set_len(size as u64).map_err(|e| {
                    CursorError::PointerFailed(format!("Failed to set file length: {}", e))
                })?;

                // Fill with opaque blue color
                use std::io::{Seek, Write};
                let file_clone = temp_file.try_clone().unwrap();
                let mut file = std::fs::File::from(file_clone);
                file.seek(std::io::SeekFrom::Start(0)).unwrap();

                let barely_visible_pixel = [0x80u8, 0x00, 0x00, 0x00]; // ARGB: barely visible (almost transparent)
                let chunk_size = 1024;
                let chunk_data: Vec<u8> = barely_visible_pixel.repeat(chunk_size);
                let total_pixels = (width * height) as usize;
                let chunks = total_pixels / chunk_size;
                let remainder = total_pixels % chunk_size;

                for _ in 0..chunks {
                    file.write_all(&chunk_data).unwrap();
                }
                if remainder > 0 {
                    file.write_all(&barely_visible_pixel.repeat(remainder))
                        .unwrap();
                }
                file.flush().unwrap();
                log::debug!("Filled buffer with {} pixels", total_pixels);

                // Create pool and buffer
                let fd = unsafe { BorrowedFd::borrow_raw(temp_file.as_raw_fd()) };
                let pool = shm.create_pool(fd, size, &self.queue.handle(), ());
    ...
    fn event(
        ...
        match event {
            wl_output::Event::Geometry { x, y, .. } => {
                if let Some((_, output_info)) = state
                    .outputs
                    .iter_mut()
                    .find(|(o, _)| o.id() == output.id())
                {
                    output_info.x = x;
                    output_info.y = y;
                    log::debug!("Output geometry updated: x={}, y={}", x, y);
                }
            }
            wl_output::Event::Mode { width, height, .. } => {
                    ...
                    .find(|(o, _)| o.id() == output.id())
                {
                    ...
                    log::debug!("Output mode updated: {}x{}", width, height);
                }
            }
            _ => {}
        }
    ...
    fn event(
        ...
        let new_position = match event {
            wl_pointer::Event::Motion {
                ...
                if let Some((_output, output_info)) = state.outputs.first() {
                    let global_x = output_info.x + surface_x as i32;
                    let global_y = output_info.y + surface_y as i32;

                    Some(CursorPosition {
                        x: global_x,
                        y: global_y,
                        output_x: output_info.x,
                        output_y: output_info.y,
...
impl<'a> CursorTrackerUpdater<'a> {
    pub fn update_input_regions(&mut self, global_x: i32, global_y: i32) -> Result<(), CursorError> {
...
impl CursorTracker {
    pub fn process_pending_input_updates(&mut self) -> Result<(), CursorError> {
        if let Some(position) = self.state.current_position {
            log::debug!("Processing input region updates for position ({}, {})", position.x, position.y);
    ...
    fn event(
        ...
        match event {
            zwlr_layer_surface_v1::Event::Configure {
                ...
                log::debug!(
                    "Layer surface configured: {}x{} (total configured: {})",
                    width,
                    height,
                    ...
                    .position(|(ls, _)| ls.id() == layer_surface.id())
                {
                    if let (Some(surface), Some(buffer)) = (
                        ...
                        let (_output, output_info) = &state.outputs[surface_idx];
                        ...
                        surface.commit();
                        log::info!(
                            "Attached buffer and committed surface - blue overlay should now be visible! ({}x{})",
                            width,
                            height
                        );
                    } else {
...
pub fn monitor_cursor_position<F>(stop_sig: Arc<AtomicBool>, callback: F) -> Result<(), CursorError>
where
...
pub fn get_cursor_position() -> Result<Option<CursorPosition>, CursorError> {
    let mut tracker = CursorTracker::new()?;
    tracker.start_tracking()?;
