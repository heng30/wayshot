...
pub struct MagnifierFilter {
    ...
    /// Magnifier radius in pixels (based on 1080p)
    /// The magnifier is a fixed-size circle
    #[derivative(Default(value = "100"))]
    pub radius: u32,

    /// Scale factor (1.0 = no magnification, 2.0 = 2x magnification)
...
impl MagnifierFilter {
    pub const NAME: &'static str = "magnifier";

    pub fn new(center_x: f32, center_y: f32, radius: u32, scale: f32) -> Self {
        Self {
            center_x,
            center_y,
            radius,
    ...
    pub fn animatable_properties() -> Vec<AnimatableProperty> {
        vec![
            ...
            AnimatableProperty::float("radius", "Radius", 0.0, 5000.0, 100.0),
    ...
    pub fn get_values_at_time(&self, time_ms: i64) -> MagnifierValues {
    ...
    fn bilinear_sample(source: &RgbaImage, x: f64, y: f64) -> Option<Rgba<u8>> {
        ...
        // Check if the sampling position is within bounds
    ...
    /// Check if a point is inside a circle with anti-aliasing
    #[inline]
    ...
    fn apply_magnifier(values: &MagnifierValues, buffer: &mut RgbaImage) -> Result<()> {
        let width = buffer.width();
        let height = buffer.height();

        if values.radius == 0 || values.scale <= 1.0 {
            return Ok(());
        }
        ...
        let scale = values.scale as f64;

        // Create a copy of the original image for sampling
        let source = buffer.clone();

        // Process all pixels within the magnifier radius
            ...
            .map(|y| {
                (x_start..x_end)
                    .map(|x| {
                        ...
                        // Check if inside magnifier circle
                        let alpha = Self::is_inside_circle(px, py, cx, cy, radius);

                        if alpha > 0.0 {
                            // Inside magnifier: sample from original at smaller scale
                            // The source position is closer to the center (inverse of scale)
                            let src_x = cx + dx / scale;
                            let src_y = cy + dy / scale;

                            // Sample from source with bilinear interpolation
                            // Returns None if outside source bounds (transparent)
                            let sampled_pixel = Self::bilinear_sample(&source, src_x, src_y);

                            match sampled_pixel {
                                Some(pixel) => {
                                    // Apply circle mask alpha
                                    let final_alpha = (pixel[3] as f64 * alpha).min(255.0) as u8;
                                    [pixel[0], pixel[1], pixel[2], final_alpha]
                                }
                                None => {
                                    // Outside source bounds - transparent
                                    // Blend with original pixel
                                    let orig = source.get_pixel(x, y);
                                    let orig_alpha = orig[3] as f64 / 255.0;
                                    let blend_alpha = 1.0 - alpha;

                                    if blend_alpha > 0.0 && orig_alpha > 0.0 {
                                        let out_alpha = orig_alpha * blend_alpha;
                                        [
                                            ((orig[0] as f64 * out_alpha / orig_alpha)
                                                .clamp(0.0, 255.0)
                                                as u8),
                                            ((orig[1] as f64 * out_alpha / orig_alpha)
                                                .clamp(0.0, 255.0)
                                                as u8),
                                            ((orig[2] as f64 * out_alpha / orig_alpha)
                                                .clamp(0.0, 255.0)
                                                as u8),
                            ...
                            // Outside magnifier - keep original pixel
        ...
        // Draw border on the magnifier circle
...
impl VideoFilter for MagnifierFilter {
    crate::impl_default_video_filter!(MagnifierFilter);
    ...
    fn apply(&self, data: &mut VideoData) -> Result<()> {
        ...
        let scaled_values = MagnifierValues {
        ...
        for frame in data.frames.iter_mut() {
            if let VideoImage::Image { buffer, .. } = frame {
                Self::apply_magnifier(&scaled_values, buffer)?;
            }
        }
