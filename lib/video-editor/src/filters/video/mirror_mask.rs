...
impl MirrorMaskFilter {
    pub const NAME: &'static str = "mirror mask";
    ...
    fn calculate_mask_factor(
        ...
        // Mirror mask: a band extending across the full frame height.
        // Only check the width direction (rx); ignore ry since band is infinite in that direction.
        let sx = rx.abs() - hw_px;
        // Normalize to consistent feather width across resolutions
        let outside_dist = sx / width_px.max(height_px);

        // By default (flip=false): inside band = visible (1.0), outside = masked
...
impl VideoFilter for MirrorMaskFilter {
    crate::impl_default_video_filter!(MirrorMaskFilter);
