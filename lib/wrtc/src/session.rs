use super::{
    DataSender, EventSender, TStreamHandler,
    errors::{SessionError, StreamError},
};
...
use commonlib::{
    ...
    http::{
        HttpRequest,
        // HttpResponse, parse_content_length
    },
    utils::Uuid,
};
use std::{
...
impl WebRTCServerSession {
    pub fn new(stream: TcpStream, event_producer: EventSender, auth: Option<Auth>) -> Self {
...
impl TStreamHandler for WebRTCStreamHandler {
    async fn send_prior_data(&self, _data_sender: DataSender) -> Result<(), StreamError> {
        Ok(())
    }
}
