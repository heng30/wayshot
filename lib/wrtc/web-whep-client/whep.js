...
class WHEPClient extends EventTarget {
  ...
  async view(pc, url, token = null) {
    ...
    try {
      ...
      // 7. 发送 offer 到服务器
      // 注意：必须发送 setLocalDescription 之后的 pc.localDescription.sdp，
      // 而不是 createOffer 返回的 offer.sdp —— 后者是快照，永远不会包含
      // 随后收集到的 ICE candidate（a=candidate 行），会导致服务器端
      // ICE 无法配对（no candidate pairs）。
      const headers = {
  ...
  async restartIce() {
    ...
    try {
      ...
      // 发送新的 offer（同样使用 pc.localDescription.sdp 以包含完整 ICE candidate）
      const headers = {
        "Content-Type": "application/sdp",
      };
