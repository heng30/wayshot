use denoise::{DenoiseError, RealTimeDenoise, denoise_model};
use hound::WavReader;
use std::sync::{
    Arc,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let reader = WavReader::open("data/ref.wav")?;
    ...
    if !processed_samples.is_empty() {
        log::info!("Saving processed audio to data/output_real_time.wav...");

        let mut writer = hound::WavWriter::create("data/output_real_time.wav", spec)?;
        ...
        log::info!("Output file saved: data/output_real_time.wav");
    }
