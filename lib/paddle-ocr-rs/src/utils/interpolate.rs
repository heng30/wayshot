...
fn compute_1d_coords(
    ...
    if input_size == 0 {
        return Err(Error::InvalidInterpSize("input_size".to_string()));
    }
    if output_size == 0 {
        return Err(Error::InvalidInterpSize("output_size".to_string()));
    }
    if input_size == 1 {
...
/// Bilinear interpolation for 4D tensor [b, c, h, w]
/// Equivalent to torch.nn.functional.interpolate with mode='bilinear'
pub fn bilinear_interpolate(
    img: &Tensor,
    out_h: usize,
    out_w: usize,
) -> Result<Tensor, Error> {
    interpolate_bilinear(img, (out_h, out_w), Some(false), None)
}

/// Interpolate 4D tensor using bilinear interpolation
pub fn interpolate_bilinear(
    ...
    if input.rank() != 4 {
        return Err(Error::InvalidInterpRank);
    }
...
/// Interpolate 4D tensor to target size using bilinear interpolation
pub fn interpolate(
    img: &Tensor,
    out_h: usize,
    out_w: usize,
    ...
    bilinear_interpolate(img, out_h, out_w)
}