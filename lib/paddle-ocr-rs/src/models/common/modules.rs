...
use candle_nn::{
    ...
    VarBuilder, conv2d, conv2d_no_bias, layer_norm, linear_b, rms_norm,
};
...
impl TwoLinearMLP {
    ...
    pub fn forward(&self, xs: &Tensor) -> Result<Tensor, crate::Error> {
        let xs = xs
            .apply(&self.linear1)?
            .apply(&self.act)?
            .apply(&self.linear2)?;
...
impl NaiveAttention {
    ...
    pub fn forward_with_cache(
        ...
        let value_states = value_states
            .reshape((b_sz, q_len, self.num_kv_heads, self.head_dim))?
            .transpose(1, 2)?;
        let (query_states, key_states) = match (cos, sin) {
            (Some(c), Some(s)) => apply_rotary_pos_emb(&query_states, &key_states, c, s, tof32)?,
            _ => (query_states, key_states),
        };
...
/// Naive Attention + Two Linear MLP Block
#[derive(Debug, Clone)]
...
/// Naive Attention + Gate Up-Down MLP Block
#[derive(Debug, Clone)]
...
impl NaiveAttnGateUpDownMLPBlock {
    pub fn new(
        ...
        let post_attention_layernorm =
            rms_norm(hidden_size, norm_eps, vb.pp(post_norm_pp_name))?;
        Ok(Self {
            self_attn,
            mlp,
            input_layernorm,
...
/// Get LayerNorm layer
pub fn get_layer_norm(vb: VarBuilder, eps: f64, dim: usize, affine: bool) -> Result<LayerNorm, crate::Error> {
