...
use candle_core::{IndexOp, Shape, Tensor};
use candle_nn::{
...
/// Projector module for vision-language alignment
...
impl Projector {
    pub fn new(vb: VarBuilder, config: &PaddleOCRVLConfig) -> Result<Self, crate::Error> {
    ...
    pub fn forward(&self, xs: &Tensor, image_grid_thw: &Tensor) -> Result<Tensor, crate::Error> {
        let img_num = image_grid_thw.dim(0)?;
        let mut processed_features = vec![];
        let start = 0usize;
        for i in 0..img_num {
            let [t, h, w] = image_grid_thw.i(i)?.to_vec1::<u32>()?[..] else {
                return Err(crate::Error::InvalidGridThw);
            };
...
impl SiglipVisionEmbeddings {
    pub fn new(vb: VarBuilder, config: &PaddleOCRVLVisionConfig) -> Result<Self, crate::Error> {
    ...
    fn interpolate_pos_encoding(
        ...
        let patch_pos_embed = self
            ...
            .permute((0, 3, 1, 2))?;
        let patch_pos_embed =
            bilinear_interpolate(&patch_pos_embed, new_height, new_width)?;
        let patch_pos_embed =
            patch_pos_embed
                .permute((0, 2, 3, 1))?
    ...
    pub fn forward(
        ...
        let mut embeddings = patch_embeds.squeeze(candle_core::D::Minus1)?.squeeze(candle_core::D::Minus1)?;
        if interpolate_pos_encoding {
            ...
            embeddings = Tensor::cat(&tmp_embeddings, 0)?.unsqueeze(0)?;
        } else {
...
impl PaddleOCRVLModel {
    pub fn new(cfg: PaddleOCRVLConfig, vb: VarBuilder, eos_ids: Vec<u32>) -> Result<Self, crate::Error> {
        ...
        if image_grid_thw.is_some() || video_grid_thw.is_some() {
            ...
            for i in 0..total_input_ids.dim(0)? {
                ...
                match vision_indices {
                    Ok(indices) => {
                        let vision_tokens = input_ids_i.gather(&indices, 0)?.to_vec1::<u32>()?;
                        let vision_indices_vec = indices.to_vec1::<u32>()?;
            ...
            let mut mrope_position_deltas = position_ids
                ...
                .broadcast_sub(&Tensor::new(
                    vec![mask.dim(candle_core::D::Minus1)? as u32 - 1],
                    input_ids.device(),
                )?)?
                .contiguous()?;
            if mrope_position_deltas.rank() == 1 {
            ...
            let position_ids =
                Tensor::arange(0_u32, input_ids.dim(candle_core::D::Minus1)? as u32, input_ids.device())?
                    .unsqueeze(0)?
                    .unsqueeze(0)?
                    .broadcast_as((3, input_ids.dim(0)?, input_ids.dim(candle_core::D::Minus1)?))?
                    .contiguous()?;
    ...
    pub fn forward(
        ...
        // Process pixel_values if all three are present
        match (pixel_values, image_grid_thw, image_mask) {
            (Some(pv), Some(igt), Some(im)) => {
                let pixel_values = pv.unsqueeze(0)?;
                let mut siglip_position_ids = vec![];
                let mut cu_seqlens = vec![0u32];
                let img_num = igt.dim(0)?;
                for i in 0..img_num {
                    let [t, h, w] = igt.i(i)?.to_vec1::<u32>()?[..] else {
                        return Err(crate::Error::InvalidGridThw);
                    };
                    let numel = h * w;
                ...
                let image_embed =
                    self.visual
                        .forward(&pixel_values, igt, &siglip_position_ids, true)?;
                let image_embed = image_embed.squeeze(0)?;
                let image_embed = self.mlp_ar.forward(&image_embed, igt)?;
                inputs_embeds = masked_scatter_dim0(&inputs_embeds, &image_embed, im)?;
            }
            _ => {}
        }
        let position_ids;
            ...
            || self.rope_deltas.is_none()
        {
            ...
            let delta = match (cache_position, &self.rope_deltas) {
                (Some(cp), Some(rd)) => {
                    cp.i(0)?
                        .to_dtype(rd.dtype())?
                        .broadcast_add(rd)?
                ...
                _ => Tensor::zeros(1, inputs_embeds.dtype(), inputs_embeds.device())?,
            };
...
impl InferenceModel for PaddleOCRVLModel {
    fn forward_initial(
        ...
        if data.data_vec.len() != 4 {
            return Err(crate::Error::InvalidMultiModalData);
        }
