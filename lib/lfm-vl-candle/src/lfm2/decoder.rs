//! LFM2 language model decoder with hybrid attention/conv layers

use crate::lfm2::config::Lfm2Config;
use crate::rope::RoPE;
use crate::util::tensor_utils::{conv1d_depthwise, prepare_causal_attention_mask};
use crate::util::modules::{
    GateUpDownMLP, QKNormAttention, get_conv1d,
};
use crate::error::Result;
use candle_core::{D, Tensor};
use candle_nn::{Conv1d, Embedding, Linear, Module, RmsNorm, VarBuilder, embedding, linear_b, rms_norm};
...
impl Lfm2ShortConv {
    ...
    pub fn forward(&mut self, xs: &Tensor) -> Result<Tensor> {
        ...
        let conv_out = if self.cache.is_none() && seq_len > 1 {
            ...
            let new_conv_state = Tensor::cat(&[conv_state, &bx], D::Minus1)?;
            let conv_state = new_conv_state.narrow(D::Minus1, 1, self.l_cache)?;
...
// ── LFM2 text-only model ────────────────────────────────────────────

pub struct Lfm2Model {
    pub model: Lfm2Decoder,
    pub lm_head: Linear,
