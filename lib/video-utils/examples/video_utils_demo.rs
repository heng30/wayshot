...
fn main() {
    ...
    println!("=====================================");
    match extract_audio_interval(&test_video, 1.0, 3.0) {
        Ok(audio) => {
            println!("✓ 成功提取音频数据");
            println!("  采样率: {} Hz", audio.sample_rate);
            println!("  声道数: {}", audio.channels);
            println!("  样本格式: {}", audio.sample_format);
            println!("  开始时间: {:.2} 秒", audio.start_time);
            println!("  持续时间: {:.2} 秒", audio.duration);
    ...
    println!("=====================================");
    match extract_frames_interval(&test_video, 1.0, 4.0, 1.0) {
        Ok(frames) => {
            println!("✓ 成功提取 {} 帧", frames.len());
            for (i, frame) in frames.iter().take(3).enumerate() {
                println!("  帧 {}: {}x{}, 时间: {:.2}s, 大小: {} bytes",
                    i + 1, frame.width, frame.height, frame.pts, frame.data.len());

                // Save first few frames as images
                if i < 3 {
                    let output_path = PathBuf::from(format!("tmp/frame_{}_at_{:.1}s.png", i + 1, frame.pts));
                    match save_frame_as_image(frame, &output_path) {
                        Ok(_) => println!("    已保存到: {:?}", output_path),
    ...
    match extract_all_audio(&test_video) {
        Ok(audio) => {
            ...
            println!("  采样率: {} Hz", audio.sample_rate);
            println!("  声道数: {}", audio.channels);
            println!("  总时长: {:.2} 秒", audio.duration);
        }
        Err(e) => {
            eprintln!("✗ 提取音频失败: {}", e);
        }
