...
impl MusicGenAudioEncodec {
    pub fn encode(
        ...
        let input_tensor = Tensor::from_array(arr.into_owned())?;
        let mut outputs = self.audio_encodec_decode.run(ort::inputs![input_tensor])?;
        ...
        Err(ort::Error::new("Token stream must be either f16 or f32"))
    }
}
